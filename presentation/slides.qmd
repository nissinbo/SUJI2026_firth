...
分離と呼ばれる状況では、通常の最尤法で有限の回帰係数を求めることができない。
...
この条件では、Firth法によってバイアスが大きく低減している。
...
#presentation-table(
  ...
  [R brglm2], [Wald区間], [Wald検定], [本発表ではWald法を使用],
)
```
...
**今回の各群25例の比較では、PL区間に揃えると比較可能な674表すべてで判定が一致した。**
...
**今回の比較では、Wald区間に揃えても標準誤差の計算方法の違いにより判定差が残った。**\
...
**今回の全列挙では、総症例数が増えるほど判定が異なった割合は低下した。**
...
**今回の4条件では、総症例数が同じ場合に群配分1:4の方が判定差の割合は高かった。**

## 共変量モデルでの信頼区間の判定差

![](../results/figures/covariate_ci_decision.png){width=72%}

**今回の8条件では、共変量の相関が高くなるほど判定差が増える傾向は見られなかった。**
...
**信頼区間・検定：** %FL・logistfはPL法とWald法を切り替えられる。本発表で用いたbrglm2の出力はWald法である。\
...
**まず推論方式を揃え、Wald法では標準誤差の定義も確認する。さらに、点推定・区間計算それぞれの収束設定と実際の収束状態を確認する。**
